...
where
 ...
 -- =========================================================================
 -- Query 3: Find policies not associated with any alerts.
 -- =========================================================================
...
from
    policies
inner join
    alerts on policies.policy_id = alerts.policy_id
group by
    policies.name, policies.policy_id
order by
    total_alerts desc, policies.name;
...
select
    ...
    users.department,
    count(alerts.alert_id) as total_alerts,
    count(distinct alerts.policy_id) as distinct_policies_violated
from
    users
inner join
    alerts on users.user_id = alerts.user_id
...
having
    count(distinct alerts.policy_id) > 1
order by
    distinct_policies_violated desc;
